...
def Run(argspath="./"):
    ...
    input2=f"{argspath}{tempdat}"
    rawfile = open('%s'%(input1),'rb')
    f=open('%s'%(input2),'r')
    ...
    for meb in [0b001, 0b010, 0b100, 0b111]:
        ...
        plot_map(argspath,'%s-unmaskable-meb%d.png'%(outfilename,meb),'Unmaskable pixels (fire if pulsed but also masked) MEB%d'      %meb,unmaskable[meb])
        plot_map(argspath,'%s-unpulsable-meb%d.png'%(outfilename,meb),'Unpulsable pixels (do not fire if not masked but pulsed) MEB%d'%meb,unpulsable_meb)
        plot_map(argspath,'%s-stuck-meb%d.png'     %(outfilename,meb),'Stuck pixels (fire if not masked and not pulsed) MEB%d'        %meb,stuck[meb])
        plot_map(argspath,'%s-bad-meb%d.png'       %(outfilename,meb),'Bad pixels (fire if masked and not pulsed) MEB%d'              %meb,bad[meb])
        plot_map(argspath,'%s-masked-meb%d.png'    %(outfilename,meb),'Masked pixels (due to failure) MEB%d'                          %meb,masked[meb])
    ...
    plot_map(argspath,'%s-unmaskable-all.png'%outfilename,'Unmaskable pixels (fire if pulsed but also masked)'      ,unmaskable_all)
    plot_map(argspath,'%s-unpulsable-all.png'%outfilename,'Unpulsable pixels (do not fire if not masked but pulsed)',unpulsable_all)
    plot_map(argspath,'%s-stuck-all.png'%outfilename     ,'Stuck pixels (fire if not masked and not pulsed)'        ,stuck_all)
    plot_map(argspath,'%s-bad-all.png'%outfilename       ,'Bad pixels (fire if masked and not pulsed)'              ,bad_all)
    plot_map(argspath,'%s-masked-all.png'%outfilename    ,'Masked pixels (due to failure)'                          ,masked_all)
